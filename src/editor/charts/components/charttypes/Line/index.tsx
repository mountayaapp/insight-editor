...
import { generateLineSeriesOptions } from "./utils";
...
export const LineChart = (props: LineChartProps) => {
	...
	const [currentSeries, setCurrentSeries] = useState<LineSeriesOption[]>(() => {
		const initialDataForX = generateSeries(props.dataset.dataContainer, fieldForX);
		if (!fieldForX || !fieldForY) {
			return [];
		}

		const { series } = generateLineSeriesOptions(
			props.dataset,
			fieldForX,
			fieldForY,
			fieldForGroupBy,
	...
	const updateChartDataAndNotifyParent = useCallback(
		(
			...
			let newSeries: LineSeriesOption[] = [];
